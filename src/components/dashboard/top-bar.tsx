...
export function TopBar({ userName, userEmail, pageTitle }: TopBarProps) {
  ...
  return (
    <header className="h-12 bg-white border-b border-zinc-100 flex items-center justify-between px-6 flex-shrink-0">
      {pageTitle ? (
        <h1 className="text-sm font-semibold text-zinc-800">{pageTitle}</h1>
      ) : (
        <div />
      )}
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={wrapperRef} className="relative hidden md:block">
          <div className="relative">
            ...
            <input
              ...
              className="w-52 border border-zinc-200 rounded-lg pl-9 pr-8 py-1.5 text-[13px] outline-none focus:ring-2 focus:ring-[#1B2B4B]/10 focus:border-zinc-400 bg-zinc-50 focus:bg-white transition-colors placeholder:text-zinc-400"
            />
            {query && (
              <button
                onClick={clearSearch}
          ...
          {open && (
            <div className="absolute top-full mt-1.5 left-0 w-72 bg-white rounded-lg border border-zinc-200 shadow-md z-50 overflow-hidden">
              {searching && (
        ...
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 bg-[#1B2B4B] rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-white text-[11px] font-semibold">{getInitials(userName)}</span>
          </div>
