...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#f8fafc">
  ...
  <div style="background:white;padding:40px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,.06)">
    ...
    <p style="color:#64748b;margin:0 0 32px">Bonjour${greeting}, voici vos performances pour la semaine du ${new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}.</p>
    ...
    <a href="${process.env.NEXT_PUBLIC_APP_URL ?? 'https://app.mandato.fr'}/dashboard"
    ...
    await sendEmail({ to: ownerProfile.email, subject, html, text })
    sent++
  }
